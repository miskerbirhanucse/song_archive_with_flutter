...
class SlowRock extends StatefulWidget {
  const SlowRock({Key? key}) : super(key: key);

  @override
  _SlowRockState createState() => _SlowRockState();
}

class _SlowRockState extends State<SlowRock> {
  ...
  getAllSongs() async {
    var songs = await _songController.readSongByStyle('SlowRock');
    songs.forEach((song) {
      setState(() {
        var songModel = Song();
